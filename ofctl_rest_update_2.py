...
class StatsController(ControllerBase):
    ...
    def get_port_desc(self, req, dpid, **_kwargs):
        ...
        #                 controller=StatsController,action='update_all_flowtable',
        #                 conditions=dict(method=['POST']))

    def update_all_flowtable(self,req,**_kwargs):
        update_table = ast.literal_eval(req.body)
        print update_table
        for i in range (1,5):
            update[i]=[]
            flow1={'priority': 1, 'cookie': 0, 'actions': [{'type': 'OUTPUT', 'port': 2}], 'match': {'in_port': 1}, 'dpid': i}
            flow2={'priority': 1, 'cookie': 0, 'actions': [{'type': 'OUTPUT', 'port': 1}], 'match': {'in_port': 2}, 'dpid': i}
            update[i].append(flow1)
            update[i].append(flow2)

        for j in range(0,2):
            dpid = update[1][j].get('dpid')
            dp = self.dpset.get(int(dpid))
            cmd = dp.ofproto.OFPFC_ADD
            _ofp_version = dp.ofproto.OFP_VERSION
            _ofctl = supported_ofctl.get(_ofp_version, None)
            _ofctl.mod_flow_entry(dp, update[1][j], cmd)
        print "update s1 flowtable"
        self._request_stats(dp)
        print "send request to s1"
...
class RestStatsApi(app_manager.RyuApp):
    ...
    def _update_other_switches(self, ev):
        if ev.msg.datapath.id == 1:
            print "get feedback from s1"
            for i in range(2,5):
                for j in range(0,2):
                    dpid = update[i][j].get('dpid')
                    dp = self.dpset.get(int(dpid))
                    cmd = dp.ofproto.OFPFC_ADD
                    _ofp_version = dp.ofproto.OFP_VERSION
                    _ofctl = supported_ofctl.get(_ofp_version, None)
                    _ofctl.mod_flow_entry(dp, update[i][j], cmd)
                self.logger.info ("update s%d flowtable",i)
